...
try:
    '''
    df = Dbf5('Demo/Data1/pri_hd.dbf', codec='ascii')
    dbf=df.to_dataframe()
    dbf.to_sql(name='pr_sysky', con=conn, if_exists='append')
    print(dbf)
    
    table = dbf.Table('Demo/Data1/pri_hd.dbf', codepage='ascii')
    print(table)
    table.open(dbf.READ_WRITE)
    
    table.codepage = dbf.CodePage('ascii') # for example
    df = pd.DataFrame(table)
    table.close()
    print(df)
    df = Dbf5('Demo/Data1/pri_hd.dbf')
    csvfile = df.to_dataframe()
    print(csvfile)'''
    '''
    csvfile.to_sql(name='pr_sysky', con=conn, if_exists='append')
    cur.execute("SELECT SYSVAL FROM pr_sysky WHERE SYSKEY = 'WOLINE4'")
    g = cur.fetchall()
    print(g)'''

    df = DBF('Demo/Data1/pri_hd.dbf', char_decode_errors='replace')
    framei = DataFrame(iter(df))
    
    framei.to_sql(name='pri_hd', con=conn, if_exists='append')
    dbf1 = DBF('Demo/Data1/pri_det.dbf', char_decode_errors='ignore')
    frame = DataFrame(iter(dbf1))
    frame.to_sql(name='pri_det', con=conn, if_exists='append')
    cur.execute("SELECT OD_ITEM FROM pri_det")
    result = cur.fetchall()
    print(result)

    
    table = dbf.Table('Demo/Data1/pri_hd.dbf')
...
if ftype == 'INV':
                ...
                df1 = DBF('Demo/Data1/pri_det.dbf', char_decode_errors='replace')
                ...
                df = DBF('Demo/Data1/prq_hd.dbf', char_decode_errors='replace')
                framei = DataFrame(iter(df))
                framei.to_sql(name='prq_hd', con=conn, if_exists='append')

                df1 = DBF('Demo/Data1/prq_det.dbf', char_decode_errors='replace')
                frame1 = DataFrame(iter(df1))
                ...
                df = DBF('Demo/Data1/prw_hd.dbf', char_decode_errors='replace')
                framei = DataFrame(iter(df))
                framei.to_sql(name='prw_hd', con=conn, if_exists='append')

                df1 = DBF('Demo/Data1/prw_det.dbf', char_decode_errors='replace')
                frame1 = DataFrame(iter(df1))
